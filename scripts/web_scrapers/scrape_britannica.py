"""
scrape_britannica.py
--------------------
Scrape Britannica pages with Firecrawl.
Save raw markdown to `scraped_data/raw/`.
Save cleaned markdown to `scraped_data/processed/`.

Usage:
    from scrape_britannica import scrape_all, TARGETS
...
TARGETS = [
    {
        ...
        "filename": "pittsburgh_britannica.md",
    },
]
...
SCRAPE_OPTIONS = {
    ...
    # Exclude common page chrome and Britannica UI blocks.
    "excludeTags": [
        "nav", "footer", "aside",
        ".md-login-inline",       # subscription banner
        ".topic-ad-content",      # ad block
        ".subscribe-banner",
        ".quiz-widget",
        ".fact-box",              # side fact panel
    ],
    "waitFor": 2000,              # allow slower JS rendering
}
...
def clean_markdown(text: str) -> str:
    ...
    # Remove Britannica-specific boilerplate lines.
    noise_patterns = [
        r'^\s*Subscribe.*$',
        r'^\s*See all media.*$',
        r'^\s*Read More.*$',
        ...
        r'^\s*Britannica Quiz.*$',
        r'^\s*\*\*Britannica.*\*\*\s*$',
        r'^\s*Written and fact-checked by.*$',
        r'^\s*The Editors of Encyclopaedia.*$',
        r'^\s*Last Updated:.*$',
        r'^\s*Cite.*$',
        r'^\s*Share.*$',
        r'^\s*Give Feedback.*$',
        r'^\s*External Websites.*$',
    ]
